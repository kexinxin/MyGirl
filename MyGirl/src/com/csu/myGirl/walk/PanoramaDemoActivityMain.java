 ...
 * 全景Demo主Activity
 */
public class PanoramaDemoActivityMain extends Activity implements
		PanoramaViewListener {

	private static final String LTAG = PanoramaDemoActivityMain.class
			.getSimpleName();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.map_activity_panorama_main);
	...
	private void processType(int type) {
		if (type == BMapApiDemoMain.PID) {
			...
			mSwitchBtn.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					double lat = Double.parseDouble(mEditLat.getText()
							.toString().trim());
					double lon = Double.parseDouble(mEditLon.getText()
							.toString().trim());
			...
			mSwitchBtn.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					int x = Integer.parseInt(mEditLat.getText().toString()
							.trim());
					int y = Integer.parseInt(mEditLon.getText().toString()
							.trim());
			...
			mSSPhotoAlbumView
					.setOnIndoorAlbumClickListener(new IndoorAlbumOnClickListener() {

						@Override
						public void onItemClick(String arg0) {
							if (mPanoView != null) {
								mPanoView.setPanorama(arg0);
							}
						}
					});
			...
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					List<IndoorPanorama> indoorList = panoController
							.getIndoorPanoramaData(116.416701, 39.915291);
				}
			}).start();
		}
	}
	...
	private void addImageMarker() {
		...
		marker1.setOnTabMarkListener(new OnTabMarkListener() {
			...
			public void onTab(int arg0) {
				...
				Toast.makeText(PanoramaDemoActivityMain.this, "marker1标注已被点击",
		...
		marker2.setOnTabMarkListener(new OnTabMarkListener() {
			...
			public void onTab(int arg0) {
				Log.d(LTAG, "index:" + arg0);
				Toast.makeText(PanoramaDemoActivityMain.this, "marker2标注已被点击",
						Toast.LENGTH_SHORT).show();
			}
		});
